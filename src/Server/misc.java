...
public class misc {
     ...
     * @return The direction between the two given points. Valid directions are
     * N:0, NE:2, E:4, SE:6, S:8, SW:10, W:12, NW:14.
     */
    public static int direction(int srcX, int srcY, int destX, int destY)
    {
    	double dx=destX-srcX, dy=destY-srcY;
    	/* This might be a more elegans solution */
    	double angle = 0;
    	if(dx > 0.0)
    	{ //Right half plane
    	...
    	{ // (0,1) or (0,-1) on a circle with radius 1 u.l.
    		angle = Math.signum(dy)*90;
    	}
    	if(angle < 0.0)
    		angle += 360;
    	if(dx == 0.0 && dy == 0.0)
    		...
    		return 4;
    	else if(angle < 360.0 && angle > 315.0)
    		return 5;
    	else if(angle == 315.0)
    		return 6;
    	else if(angle < 315.0 && angle > 270.0)
    		return 7;
    	else if(angle == 270.0)
    		return 8;
    	else if(angle < 270.0 && angle > 225.0)
    		return 9;
    	else if(angle == 225.0)
    		return 10;
    	else if(angle < 225.0 && angle > 180.0)
    		return 11;
    	else if(angle == 180.0)
    		return 12;
